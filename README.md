...
- コード7.25の出力結果は、図7.51と異なる場合があります。これは、コード7.23を実行した際に割り付けられるクラスターラベル（0, 1, 2, 3, 4）がPythonの実行環境により異なるためです。7章の分析では、各会員がどのクラスターに属するかがポイントであり、K-meansのラベル（0, 1, 2, 3, 4）そのものは意味をもちません。このように、クラスターラベル（0, 1, 2, 3, 4）は、単にクラスターを区別するためだけの番号であるため、図7.51とはラベルが異なっていても、クラスタリング結果としては問題ありません。
